...
const AdminTable = () => {
    ...
    //for lock all button
    const handleLockAll = () => {
        const allLocked = employees.every(emp => emp.locked); 

        setEmployees(prevEmployees =>
            prevEmployees.map(emp => ({ ...emp, locked: !allLocked })) 
        );
    };
            ...
            <tr key={emp.id}>
                ...
                <td>
                    <input
                        ...
                        style={{ height: '50px', borderRadius: '0', fontSize: '20px' }}
